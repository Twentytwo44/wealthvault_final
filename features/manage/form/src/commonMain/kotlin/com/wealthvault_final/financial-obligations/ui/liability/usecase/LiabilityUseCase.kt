package com.wealthvault_final.`financial-obligations`.ui.liability.usecase
...
class AddLiabilityUseCase(
    ...
    override fun execute(parameters: LiabilityRequest): Flow<FlowResult<LiabilityData>> = flow {
        println("🚀 [AddLiabilityUseCase] เริ่มต้นบันทึกหุ้น: ${parameters.name}")
        ...
        result.onSuccess { cash ->
            println("✅ [AddLiabilityUseCase] บันทึกสำเร็จ: ${cash.name}")

            // 2. ส่งข้อมูล Investment (Domain Model) กลับไปให้ UI ผ่าน FlowResult
            emit(FlowResult.Continue(cash))
        }.onFailure { exception ->
            println("❌ [AddLiabilityUseCase] บันทึกไม่สำเร็จ: ${exception.message}")

            // 3. ส่ง Error กลับไป
            emit(FlowResult.Failure(exception))
        }
    }.catch { cause ->
        println("🚨 [AddLiabilityUseCase] เกิดข้อผิดพลาดไม่คาดคิด: ${cause.message}")
