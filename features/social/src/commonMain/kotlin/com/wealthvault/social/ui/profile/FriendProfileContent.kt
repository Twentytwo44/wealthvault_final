...
fun FriendProfileContent(
    ...
    val (liabilities, assets) = remember(allItems) {
        allItems.partition { it.type?.lowercase() == "liability" }
    }
    ...
    Box(modifier = Modifier.fillMaxSize().background(LightBg)) {
        Column(
            ...
            } else if (errorMessage != null && profileData == null) {
                Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                    Column(horizontalAlignment = Alignment.CenterHorizontally) {
                        Text(errorMessage, color = redColor, textAlign = TextAlign.Center)
                        TextButton(onClick = onRetry) {
                            Text("ลองใหม่", color = themeColor)
                        }
                    }
                }
            } else {
                            ...
                            ExpandableCategoryCard(
                                ...
                                assets.forEach { item ->
                                    ...
                                    val amountValue = when(item.type?.lowercase()) {
                                        "land", "building", "account" -> formatAmount(detail?.amount ?: Money(0))
                                        "insurance" -> formatAmount(detail?.coverageAmount ?: Money(0))
                                        "investment" -> formatAmount(detail?.amount ?: Money(0))
                            ...
                            ExpandableCategoryCard(
                                ...
                                liabilities.forEach { item ->
                                    ...
                                    RealItemCard(
                                        ...
                                        amountLabel = "เงินต้น",
                                        amountValue = formatAmount(detail?.principal ?: Money(0)),
                                        onClick = { selectedItem = item }
                                    )
                                }
                            }
...
internal fun friendProfileErrorMessage(error: AppError): String = when (error) {
    AppError.Unauthorized -> "เซสชันหมดอายุ กรุณาเข้าสู่ระบบใหม่"
    AppError.NotFound -> "ไม่พบโปรไฟล์เพื่อนนี้"
    is AppError.Network -> "เชื่อมต่อเซิร์ฟเวอร์ไม่ได้ กรุณาลองใหม่"
    is AppError.Unknown -> "โหลดโปรไฟล์เพื่อนไม่สำเร็จ กรุณาลองใหม่"
}
