...
internal class SocialRepositoryImpl(
    ...
    override suspend fun getAccountById(id: String): AppResult<BankAccountData> =
        dataSource.getAccountById(id).requireValue()

    override suspend fun getBuildingById(id: String): AppResult<BuildingIdData> =
        dataSource.getBuildingById(id).requireValue()

    override suspend fun getCashById(id: String): AppResult<CashIdData> =
        dataSource.getCashById(id).requireValue()

    override suspend fun getInsuranceById(id: String): AppResult<InsuranceIdData> =
        dataSource.getInsuranceById(id).requireValue()

    override suspend fun getInvestmentById(id: String): AppResult<InvestmentIdData> =
        dataSource.getInvestmentById(id).requireValue()

    override suspend fun getLandById(id: String): AppResult<LandIdData> =
        dataSource.getLandById(id).requireValue()

    override suspend fun getLiabilityById(id: String): AppResult<LiabilityIdData> =
        dataSource.getLiabilityById(id).requireValue()
    ...
    /** A deleted/shared item is a normal domain outcome, not a process crash. */
    private fun <T> AppResult<T?>.requireValue(): AppResult<T> = when (this) {
        is AppResult.Success -> {
            val item = value
            if (item == null) AppResult.Failure(AppError.NotFound) else AppResult.Success(item)
        }
