...
class IosSecureStorage : SecureStorage {
    ...
    override suspend fun write(key: String, value: String) {
        ...
        withRetained(data) { retainedData ->
            // SecItemUpdate identifies the item by its attributes. Match
            // options such as kSecMatchLimit/kSecReturnData belong only on
            // read queries; including them here makes Keychain reject the
            // update with errSecParam on iOS simulators and devices.
            withKeyQuery(key, includeData = false, includeMatchOptions = false) { query ->
                ...
                try {
                    ...
                    if (updateStatus == errSecItemNotFound) {
                        ...
                        if (insertStatus == errSecDuplicateItem) {
                            // Another process can create the item between our
                            // update and add. Replace that value instead of
                            // silently accepting a stale token.
                            check(SecItemUpdate(query, attributes) == errSecSuccess) {
                                "Unable to update secure session value after duplicate"
                            }
                        } else {
                            check(insertStatus == errSecSuccess) {
                                "Unable to write secure session value"
                            }
                        }
                    } else {
                        check(updateStatus == errSecSuccess) {
                            "Unable to update secure session value (status=$updateStatus)"
                        }
                    }
                } finally {
    ...
    private inline fun <T> withKeyQuery(
        key: String,
        includeData: Boolean,
        includeMatchOptions: Boolean = includeData,
        block: (CFDictionaryRef?) -> T,
    ): T = withRetained(key) { account ->
        withRetained(KEYCHAIN_SERVICE) { service ->
            val query = if (includeMatchOptions) {
                cfDictionaryOf(
                    kSecClass to kSecClassGenericPassword,
                    kSecAttrService to service,
