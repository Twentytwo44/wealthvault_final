    ...
    fun build(withAuth: Boolean = true): HttpClient {
        ...
        if (withAuth && tokenStore != null) {
            client.plugin(HttpSend).intercept { request ->
                ...
                var requestAccessToken: String? = null

                if (!isAuthRoute) {
                    requestAccessToken = tokenStore.accessToken.first()
                    if (!requestAccessToken.isNullOrBlank()) {
                        request.header(HttpHeaders.Authorization, "Bearer $requestAccessToken")
                    }
                }
                ...
                if (originalCall.response.status == HttpStatusCode.Unauthorized && !isAuthRoute) {
                    ...
                    // Use the credential attached to this request, not a token
                    // read after the response. Another request may have already
                    // refreshed the session while this request was in flight;
                    // using the current store value here would make this stale
                    // 401 look like a fresh failure and could start a duplicate
                    // refresh flight.
                    val newAccessToken = refreshCoordinator?.refreshAfterUnauthorized(requestAccessToken)
